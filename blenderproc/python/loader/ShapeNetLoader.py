...
def load_shapenet(data_path: str, used_synset_id: str, used_source_id: str = "",
    ...
    selected_obj = data_path + "/" + used_synset_id + "/" + used_source_id + "/model.obj"
    #selected_obj = "/hdd2/real_camera_dataset/obj_models/train/02942699/97690c4db20227d248e23e2c398d8046/model.obj"
    ...
    obj.persist_transformation_into_mesh(location=False, rotation=False, scale=False)
...
class _ShapeNetLoader:
    ...
    def get_files_with_synset_ply(used_synset_id: str, used_source_id: str, path_to_taxonomy_file: str,
                              data_path: str) -> list:
        """ Returns a list of a .obj file for the given synset_id
        ...
        if os.path.exists(path_to_taxonomy_file):
            ...
            with open(path_to_taxonomy_file, "r", encoding="utf-8") as f:
                ...
                if not used_source_id:
                    files.extend(glob.glob(os.path.join(id_path, "*", "models", "model_normalized_scaled.ply")))
                else:
                    if not os.path.exists(os.path.join(id_path, used_source_id)):
                        raise ValueError(f"The used_source_id {used_source_id} is not correct")

                    # Using both the used_synset_id and used_source_id
                    files.append(os.path.join(id_path, used_source_id, "models", "model_normalized_scaled.ply"))
