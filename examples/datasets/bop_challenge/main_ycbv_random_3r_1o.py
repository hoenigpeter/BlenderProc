...
import numpy as np
import copy 
#python rerun.py run examples/datasets/bop_challenge/main_ycbv_random_3r_1o.py ../datasets resources/cc_textures examples/datasets/bop_challenge/output --num_scenes=1000
...
for i in range(args.num_scenes):
    ...
    sampled_distractor_bop_objs += list(np.random.choice(hb_dist_bop_objs, size=3, replace=False))
    ...
    for idx, obj in enumerate(sampled_target_bop_objs):
        ...
        if scene_cnt > 0:       
            ...
            cc_texture_from_list = original_textures[idx]
            obj.replace_materials(cc_texture_from_list)           
    ...
    for idx, obj in enumerate(target_bop_objs):
        print("ORIGINAL obj texture: ", obj.get_materials()[0].blender_obj.name)
